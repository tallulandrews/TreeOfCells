# TA: These are existing libraries for building trees
library(ape)
library(phytools)
library(phangorn)
library(ggtree)
...
calculateKL <- function(c1,c2){
  ...
  kl.av <- mean(unlist(sapply(1:nrow(c1), function(g){
    ...
    if (is.na(c1[g,'mu']) == FALSE && is.na(c2[g,'mu']) == FALSE){
      ...
      # calculate kl distance between 2 univariate gaussian distributions ## TA: The data isn't gaussian so this is not correct.
